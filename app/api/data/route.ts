...
function toDay(row: Record<string, unknown>): DayEntry {
  return {
    ...
    desF: row.des_f != null ? Number(row.des_f) : undefined,
    ...
    almF: row.alm_f != null ? Number(row.alm_f) : undefined,
    ...
    merF: row.mer_f != null ? Number(row.mer_f) : undefined,
    ...
    entrenamientoReporte: (row.entrenamiento_reporte as DayEntry["entrenamientoReporte"]) || undefined,
...
function toDayRow(day: DayEntry, userId: string) {
  return {
    ...
    entrenamiento_reporte: day.entrenamientoReporte || null,
